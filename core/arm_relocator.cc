...
{
    event_code
    make_event_code(const section_data & data, unsigned int offset, const std::string & sym, int addend) const
    {
        return lyn::event_code(
            lyn::event_code::CODE_POIN, arm_relocator::abs_reloc_string(sym, data.read<std::uint32_t>(offset) + addend),
            false);
    }
...
struct arm_data_rel32_reloc : public arm_relocator::relocatelet
{
    ...
    {
        return lyn::event_code(
            lyn::event_code::CODE_WORD, arm_relocator::rel_reloc_string(sym, data.read<std::uint32_t>(offset) + addend),
            false);
    }
...
struct arm_data_abs16_reloc : public arm_relocator::relocatelet
{
    ...
    make_event_code(const section_data & data, unsigned int offset, const std::string & sym, int addend) const
    {
        return lyn::event_code(
            ...
            arm_relocator::abs_reloc_string(sym, data.read<std::uint16_t>(offset) + addend), false);
    }
...
struct arm_data_abs8_reloc : public arm_relocator::relocatelet
{
    ...
    {
        return lyn::event_code(
            lyn::event_code::CODE_BYTE, arm_relocator::abs_reloc_string(sym, data.read_byte(offset) + addend), false);
    }
...
struct arm_thumb_b_reloc : public arm_relocator::relocatelet
{
    event_code make_event_code(const section_data &, unsigned int, const std::string & sym, int addend) const
    {
        ...
        return lyn::event_code(lyn::event_code::CODE_SHORT, { arm_relocator::b_string(value) }, false);
    }
...
struct arm_thumb_bcond_reloc : public arm_relocator::relocatelet
{
    ...
    make_event_code(const section_data & data, unsigned int offset, const std::string & sym, int addend) const
    {
        ...
        return lyn::event_code(
            lyn::event_code::CODE_SHORT,
            { arm_relocator::bcond_string((data.read<std::uint16_t>(offset) & 0xFF00), value) }, false);
    }
    ...
    void apply_relocation(section_data & data, unsigned int offset, unsigned int value, int addend) const
    {
        ...
        data.write<std::uint16_t>(
            offset, (((relocatedValue >> 1) & 0xFF) | (data.read<std::uint16_t>(offset) & 0xFF00)));
    }
...
struct arm_thumb_bl_reloc : public arm_relocator::relocatelet
{
    event_code make_event_code(const section_data &, unsigned int, const std::string & sym, int addend) const
    {
        ...
        return lyn::event_code(
            lyn::event_code::CODE_SHORT, { arm_relocator::bl_op1_string(value), arm_relocator::bl_op2_string(value) },
            false);
    }
...
struct arm_arm_b_reloc : public arm_relocator::relocatelet
{
    ...
    make_event_code(const section_data & data, unsigned int offset, const std::string & sym, int addend) const
    {
        ...
        return lyn::event_code(
            lyn::event_code::CODE_WORD, arm_relocator::b24_arm_string(data.read<uint32_t>(offset), value), false);
    }
    ...
    void apply_relocation(section_data & data, unsigned int offset, unsigned int value, int addend) const
    {
        ...
        data.write<std::uint32_t>(
            offset, (((relocatedValue >> 2) & 0xFFFFFF) | (data.read<uint32_t>(offset) & 0xFF000000)));
    }
...
{
    mRelocatelets[R_ARM_ABS32].reset(new arm_data_abs32_reloc);      // R_ARM_ABS32
    mRelocatelets[R_ARM_REL32].reset(new arm_data_rel32_reloc);      // R_ARM_REL32
    mRelocatelets[R_ARM_ABS16].reset(new arm_data_abs16_reloc);      // R_ARM_ABS16
    mRelocatelets[R_ARM_ABS8].reset(new arm_data_abs8_reloc);        // R_ARM_ABS8
    mRelocatelets[R_ARM_THM_CALL].reset(new arm_thumb_bl_reloc);     // R_ARM_THM_CALL
    mRelocatelets[R_ARM_CALL].reset(new arm_arm_bl_reloc);           // R_ARM_CALL
    mRelocatelets[R_ARM_JUMP24].reset(new arm_arm_b_reloc);          // R_ARM_JUMP24
    mRelocatelets[R_ARM_THM_JUMP11].reset(new arm_thumb_b_reloc);    // R_ARM_THM_JUMP11
    mRelocatelets[R_ARM_THM_JUMP8].reset(new arm_thumb_bcond_reloc); // R_ARM_THM_JUMP8
}
