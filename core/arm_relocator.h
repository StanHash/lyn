...
public:
    struct relocatelet
    {
        ...
        virtual event_code
        make_event_code(const section_data & data, unsigned int offset, const std::string & sym, int addend) const = 0;
        virtual void
        apply_relocation(section_data & data, unsigned int offset, unsigned int value, int addend) const = 0;

        virtual bool is_absolute() const
        {
            return true;
        }
        virtual bool can_make_trampoline() const
        {
            return false;
        }
        virtual section_data make_trampoline(const std::string & symbol, int addend) const
        {
            return section_data();
        }
    };
